...
def rotate_labelled_components(
    ...
    dendrite_mesh.vertices = (
        np.asarray(dendrite_mesh.vertices, dtype=np.float64)
        * float(scale_to_nm)
    )
    ...
    if random_orientation:
        rotation_matrix, angles_deg = random_rotation_matrix(rng=rng)
    else:
    ...
    rotated_dendrite_path = output_dir / "dendrite00.ply"
    dendrite_rotated.export(rotated_dendrite_path)
    ...
    for spine_path in spine_paths:
        ...
        spine_mesh.vertices = (
            np.asarray(spine_mesh.vertices, dtype=np.float64)
            * float(scale_to_nm)
        )
        ...
        rotated_path = output_dir / spine_path.name
        spine_rotated.export(rotated_path)
        rotated_spine_paths.append(rotated_path)

    transform_metadata = {
        "random_orientation": bool(random_orientation),
        "rotation_angles_deg_xyz": [float(v) for v in angles_deg],
        "rotation_origin_xyz_nm": [float(v) for v in origin_xyz],
...
def select_random_geometry_center(dendrite_path, rng=None):
    """Select a reproducible random FOV centre from actual dendrite geometry."""
    dendrite_mesh = trimesh.load(
        dendrite_path,
        force="mesh",
        process=False,
    ...
    vertex_index = int(rng.integers(0, len(vertices)))
    center_xyz_nm = vertices[vertex_index]
    return tuple(float(v) for v in center_xyz_nm)
